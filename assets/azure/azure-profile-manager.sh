...
EXAMPLES:
  ...
  azp add dev-someproject "43a7c7cb-e2a7-4186-8cf7-335e83a59624" "d0359952-27be-4d84-ae0f-ecc3299c546d" pub

  # Add a profile for government cloud
  azp add prod-someproject "fd85bccc-c571-4c42-ac66-b3928099ae25" "4f02fea2-2757-49bc-924d-d0e5ff1ab8a2" gov

  # List all configured profiles
  azp list

  # Switch to a specific profile (automatically exports ARM_TENANT_ID and ARM_SUBSCRIPTION_ID)
  azp switch dev-someproject
...
__azp_mk_az_profiles_dir() {
  local profile_dir=$(__azp_get_az_profiles_dir)
  if [[ ! -d "${profile_dir}" ]];
  then
  mkdir -p "${profile_dir}"
...
CONFIG_DIR=$(__azp_get_az_profiles_dir)
...
__azp_add_profile() {
  ...
  if [[ -z "$profile_name" || "$profile_name" == "-h" || "$profile_name" == "--help" || -z "$tenant_id" || -z "$subscription_id" || -z "$cloud_env" ]]; then
    print_error "Usage: azp add_profile <profile_name> <tenant_id> <subscription_id> <cloud_environment>"
